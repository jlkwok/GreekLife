...
public interface ChapterRepository extends CrudRepository<Chapter, String> {
	...
	@Query("SELECT m.name FROM Chapter c, MemberOf o, ChapterMember m WHERE m.sid=o.sid AND o.id.chapterName=c.chapterName and c.chapterName=?1")
	public List<String> getMembers(String chapterName);
	
	@Query("SELECT h.address FROM Chapter c, HouseOf o, ChapterHouse h WHERE h.address=o.address AND o.chapterName=c.chapterName and c.chapterName=?1")
	public String getHouse(String chapterName);
	
	@Query("SELECT p.id.eventName FROM Chapter c, Hosts h, Philanthropy p WHERE p.id.eventName=h.id.eventName AND p.id.date=h.id.date AND h.id.chapterName=c.chapterName and c.chapterName=?1")
	public List<String> getPhilanthropy(String chapterName);	
	
	@Query("SELECT m.name FROM Chapter c, ServesAs s, ChapterMember m WHERE m.sid=s.id.sid AND s.chapterName=c.chapterName and c.chapterName=?1")
	public List<String> getExecBoard(String chapterName);
	...
	@Query("SELECT c.chapterName FROM Chapter c WHERE NOT EXISTS (SELECT m FROM ChapterMember m, MemberOf o WHERE o.chapterName=c.chapterName AND o.sid=m.sid AND m.gpa<?1)")
	public List<String> getChaptersWithMinGpa(Double gpa);
	
	@Query("SELECT c.chapterName FROM Chapter c WHERE (SELECT AVG(m.gpa) FROM ChapterMember m, MemberOf o WHERE o.chapterName=c.chapterName AND o.sid=m.sid)>?1")
	public List<String> getChaptersWithAverageGpa(Double gpa);
	
	@Query(value = "SELECT c.chapter_name FROM chapter c "
			...
			nativeQuery=true)
	public List<String> getChaptersWithAllMembersInHouse();
